...
export function PraktikanCourseOverview({ userId, onViewClasses }: PraktikanCourseOverviewProps) {
  ...
  if (query.isPending) {
    return <div role="status" className="flex min-h-72 items-center justify-center rounded-3xl border border-slate-200 bg-white"><Loader2 className="h-5 w-5 animate-spin" aria-hidden="true" /><span className="ml-3 text-sm text-slate-600">Loading your practicum overview...</span></div>;
  }

  if (query.isError) {
    return <div role="alert" className="rounded-3xl border border-red-200 bg-red-50 p-6"><h1 className="font-semibold text-red-950">Overview could not be loaded</h1><p className="mt-2 text-sm text-red-800">Your enrolled course information is temporarily unavailable.</p><button type="button" onClick={() => void query.refetch()} disabled={query.isFetching} className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-xl bg-red-700 px-4 text-sm font-semibold text-white disabled:opacity-60"><RefreshCw className="h-4 w-4" aria-hidden="true" />Try again</button></div>;
  }
  ...
  return (
    <div className="space-y-7">
      <header>
        <p className="text-xs font-semibold uppercase tracking-wider text-emerald-700">Praktikan workspace</p>
        <h1 className="mt-1 text-2xl font-bold tracking-tight text-slate-950 sm:text-3xl">Your practicum at a glance</h1>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Continue an active class or review your enrolled academic history.</p>
      </header>
      <dl className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        {[
          { label: "Enrolled classes", value: courses.length, helper: "Across all academic periods", icon: Layers3 },
          { label: "Active classes", value: activeCourses.length, helper: "Available this period", icon: BookOpen },
          { label: "Past classes", value: courses.length - activeCourses.length, helper: "Previous offerings", icon: History },
          { label: "Attendance records", value: attendanceQuery.isError ? "—" : attendanceRows.length, helper: attendanceQuery.isError ? "Temporarily unavailable" : "Saved personal records", icon: ClipboardCheck },
          { label: "Published average", value: gradesQuery.isError || average === null ? "—" : average.toFixed(1), helper: gradesQuery.isError ? "Temporarily unavailable" : "Published session grades", icon: GraduationCap }
