...
export function PraktikanCourseGrades({ userId, courseId }: Props) {
  ...
  return <section aria-labelledby="course-grades-heading"><div><p className="text-xs font-semibold uppercase tracking-wider text-emerald-700">Course performance</p><h2 id="course-grades-heading" className="mt-1 text-xl font-semibold text-slate-950">Grades &amp; scores</h2><p className="mt-1 text-sm text-slate-600">Scores are displayed once published by your instructors.</p></div>{sessions.length === 0 ? <div role="status" className="mt-5 rounded-3xl border border-dashed border-slate-300 bg-white px-6 py-10 text-center"><GraduationCap className="mx-auto h-8 w-8 text-slate-400" aria-hidden="true" /><h3 className="mt-3 font-semibold text-slate-950">No graded sessions yet</h3><p className="mt-1 text-sm text-slate-500">Session scores will appear here once they&apos;re published.</p></div> : <div className="mt-5 grid gap-3 sm:grid-cols-2">{sessions.map((session) => { const row = rows.get(session.id); return <article key={session.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-4"><div><h3 className="font-semibold text-slate-950">{session.title}</h3><p className="mt-1 text-sm text-slate-600">{formatDate(session.date)}</p></div>{!session.grades_published ? <span className="rounded-xl bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700">Not released yet</span> : row ? <span className="rounded-xl bg-emerald-50 px-3 py-2 text-lg font-bold text-emerald-900">{formatScore(row.score)}</span> : <span className="max-w-36 rounded-xl bg-amber-50 px-3 py-2 text-right text-xs font-semibold text-amber-800">No score recorded</span>}</div></article>; })}</div>}</section>;
}
