      ...
      <div className="space-y-2.5">
        ...
        {filteredStaff.length === 0 ? (
          <p className="text-xs text-slate-400 italic py-2">
            No teaching assistants assigned to this course yet.
          </p>
        ) : (
          <div className="divide-y divide-slate-100 bg-white rounded-3xl border border-slate-200 shadow-xs">
            {filteredStaff.map((s) => (
              <div
                key={s.id}
            ...
            {filteredStudents.map((st) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <span className="text-slate-500 text-[11px] font-medium">Enrolled</span>
                  <button
                    type="button"
                    onClick={() => handleUnenroll(st)}
                    className="apple-press p-2 sm:px-2.5 sm:py-1.5 text-rose-600 hover:text-rose-700 hover:bg-rose-50 rounded-lg text-xs transition-colors flex items-center gap-1"
                    title="Unenroll student"
                    aria-label={`Unenroll student ${st.username}`}
                  >
                ...
                {tags.map((tag, idx) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="apple-press hover:bg-white/20 rounded-full p-0.5 transition-colors"
                      title="Remove"
