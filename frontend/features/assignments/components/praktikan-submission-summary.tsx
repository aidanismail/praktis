...
export function PraktikanSubmissionSummary({ submission, maxPoints }: PraktikanSubmissionSummaryProps) {
  ...
  return (
    <section aria-labelledby="my-submission-heading" className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wider text-emerald-700">Private result</p><h2 id="my-submission-heading" className="mt-1 text-lg font-semibold text-slate-950">My submission</h2></div><div className="flex gap-2"><span className={submission.is_late ? "rounded-full bg-red-50 px-2.5 py-1 text-xs font-semibold text-red-700" : "rounded-full bg-sky-50 px-2.5 py-1 text-xs font-semibold text-sky-700"}>{submission.is_late ? "Late" : "On time"}</span><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700">{submission.status}</span></div></div>
      <dl className="mt-5 grid gap-3 sm:grid-cols-2">
        <div className="rounded-2xl bg-slate-50 p-4"><dt className="text-xs font-semibold uppercase tracking-wide text-slate-500">File</dt><dd className="mt-2 wrap-break-word text-sm font-medium text-slate-950">{submission.file_name}</dd><dd className="mt-1 text-xs text-slate-500">{formatFileSize(submission.file_size)}</dd></div>
        <div className="rounded-2xl bg-slate-50 p-4"><dt className="text-xs font-semibold uppercase tracking-wide text-slate-500">Submitted</dt><dd className="mt-2 text-sm font-medium text-slate-950"><time dateTime={submission.submitted_at}>{formatDate(submission.submitted_at)}</time></dd></div>
        <div className="rounded-2xl bg-slate-50 p-4"><dt className="text-xs font-semibold uppercase tracking-wide text-slate-500">Score</dt><dd className="mt-2 text-sm font-semibold text-slate-950">{submission.score === null ? "Not graded" : `${submission.score} / ${maxPoints}`}</dd></div>
