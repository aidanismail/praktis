      ...
      <p className="text-xs font-semibold uppercase tracking-wider text-emerald-700">
        ...
        <h1 id="profile-heading" className="mt-1 text-2xl font-bold tracking-tight text-slate-950">
          ...
          <p className="mt-2 text-sm text-slate-600">
            Manage your account credentials and personal details.</p>
            </div>
                        ...
                        <dl className="mt-6 space-y-3">
                          <div className="rounded-2xl bg-slate-50 p-4">
                            <dt className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-slate-500">
                              ...
                              Email</dt>
                              <dd className="mt-2 wrap-break-word text-sm font-medium text-slate-950">{user.email}</dd></div><div className="rounded-2xl bg-slate-50 p-4">
                                <dt className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-slate-500">
                                  <ShieldCheck className="h-4 w-4" aria-hidden="true" />Account status</dt>
                                  <dd className="mt-2 text-sm font-medium text-slate-950">
                                    {user.is_active ? "Active" : "Inactive"}</dd>
                                    </div>
                                    </dl>
                                    <p className="mt-5 text-sm leading-6 text-slate-600">
                                      Student identity details are managed by your department administrators and can&apos;t be edited directly.</p>
                                      </article>
                                      <aside className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
                                        <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-slate-100 text-slate-700">
                                          ...
                                          <Link href={ROUTES.changePassword} className="mt-5 inline-flex min-h-11 items-center rounded-xl bg-emerald-700 px-4 text-sm font-semibold text-white hover:bg-emerald-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-700">
